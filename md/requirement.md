...
> Trạng thái hiện tại của dự án tính đến lần cập nhật gần nhất: **đã dựng khung folder + file rỗng theo `rules.md §1`, chưa có logic bên trong bất kỳ file nào.** Toàn bộ checklist bên dưới bắt đầu ở trạng thái 🔲.
...
| ------------------------- | ------------------------------------------------------------------------------------------------------ | ---------- | ------- |
| `index.html`, `style.css` | Đủ layout: 2 nút nguồn data, panel classic/mixed, tab so sánh                                          | �         | Đã có cấu trúc cơ bản cho chọn nguồn dữ liệu, nút Run và tab; chưa đầy đủ như kịch bản demo hoàn chỉnh |
| `app.js`                  | Cache response, không gọi API 2 lần cho progressive reveal; đọc field `is_hot` (không phải field khác) | ✅         | Đã thực hiện cache `lastResponse`, dùng lại kết quả cho tab classic/mixed/so sánh và đọc `is_hot` trong bảng so sánh |
| `charts.js`               | 3 hàm render đúng chữ ký ở `ai_agents.md`                                                              | ✅         | Đã có `renderTop10Table`, `renderErrorBarChart`, `renderDetectionBarChart` |
| —                         | Đã chạy được hoàn chỉnh bằng `mock_data.js` trước khi có backend thật                                  | ✅         | UI có fallback chạy bằng mock data khi backend chưa sẵn sàng |
| —                         | Đã swap sang `api.js` thật, test lại toàn luồng                                                        | 🟡         | Code đã sẵn sàng cho `window.api`, nhưng hiện vẫn đang dùng mock data trong giao diện mặc định |

#### Phần riêng người B — thay đổi thực tế so với yêu cầu gốc
- Đã thêm: fallback chạy bằng `mock_data.js`, cache kết quả ở frontend để tránh gọi lại API khi chuyển tab hoặc bấm nút Mixed; hiển thị nhãn nguồn dữ liệu và panel so sánh.
- Đã điều chỉnh: luồng UI dùng “Run once → xem classic → bấm Mixed” thay vì gọi API hai lần; bảng top-10 đọc field `is_hot` để phân loại Elephant/Mice.
- Bị giảm/ chưa hoàn thiện: chưa có dropdown preset đầy đủ từ `/api/presets`, chưa swap hoàn toàn sang `api.js` thật trong index mặc định, chưa có test toàn luồng với backend thật trên môi trường chạy thực tế.
- Rủi ro cần ghi nhận: nếu backend đổi schema/field, UI cần cập nhật lại ngay vì hiện tại việc render phụ thuộc trực tiếp vào `results.classic`, `results.mixed`, `is_hot` và tên `ip`.
...
| `real_loader.py`                                | Parse đúng regex `rules.md §5`; có cache; log số dòng skip                                                                   | 🔲         |                                                                                         |
...
| `test_data_generator.py`, `test_real_loader.py` | Pass 100%                                                                                                                    | 🔲         |                                                                                         |
| `backend/scripts/run_full_experiments.py`       | Quét đủ tham số, `total_packets=10,000,000`, xuất số liệu thay placeholder trong `Kịch bản DEMO`                             | 🔲         | Mốc cuối timeline (`project_ovr.md §9`)                                                 |
...
_(Thêm dòng mới mỗi khi có thay đổi — không xoá dòng cũ, kể cả khi bị từ chối, ghi rõ "Từ chối — lý do..." ở cột Trạng thái để giữ lịch sử quyết định.)_
