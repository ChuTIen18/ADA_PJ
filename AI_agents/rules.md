...
- **Tiền xử lý 1 lần, cache lại**: parse toàn bộ file log → đếm `{host: count}` → lưu ra `backend/app/data/real/processed_cache.json`. Mỗi request `/api/simulate` với `data_source=real` chỉ đọc file cache này, **không** parse lại log thô mỗi lần (file có thể vài trăm nghìn — hàng triệu dòng, parse lại mỗi request sẽ rất chậm).
